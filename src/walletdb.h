...
public:
    ...
    bool WriteZMNTSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMNTSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMNTSeed_deprecated(uint256& seed);
    bool EraseZMNTSeed();
    bool EraseZMNTSeed_deprecated();

    bool WriteZMNTCount(const uint32_t& nCount);
    bool ReadZMNTCount(uint32_t& nCount);
