                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zMNT ought to be enough for anybody." - Bill Gates, 2017
    ui->zMNTpayAmount->setValidator( new QDoubleValidator(0.0, 210000000.0, 20, this) );
    ...
    ui->labelzMNTSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMNTpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMNT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(!Params().ZeroCoinEnabled() || GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMNT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zMNT, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMNT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMNT in ") +
...
void PrivacyDialog::on_pushButtonSpendzMNT_clicked()
{
    ...
    if(!Params().ZeroCoinEnabled() || GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMNT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zMNT, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zMNT
        sendzMNT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMNT
    sendzMNT();
}

void PrivacyDialog::on_pushButtonZMntControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZMntControlDialog* zMntControl = new ZMntControlDialog(this);
    zMntControl->setModel(walletModel);
    zMntControl->exec();
}

void PrivacyDialog::setZMntControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMntSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMNT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Mnt  Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMNTpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMNT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MNT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMNT</b>";
    ...
    // use mints from zMNT selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMntControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZMntControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zMNT require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zMNT"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZMNT_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zMNT require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zMNT"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zMNT was spent successfully update the addressbook with the label
    ...
    // Clear zmnt selector in case it was used
    ZMntControlDialog::setSelectedMints.clear();
    ui->labelzMntSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMNT Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Mnt, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMNT Mint");
                               ...
                               const CAmount& earnings, const CAmount& masternodeEarnings, const CAmount& stakeEarnings)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zmntTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMNT </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMNT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMNT "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zMNT "));
    ui->labelzMNTAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zMNT </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zMNT </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zMNT </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzMNTSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzMNT->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zMNT
        ui->pushButtonMintzMNT->setEnabled(false);
        ui->pushButtonMintzMNT->setToolTip(tr("zMNT is currently disabled due to maintenance."));

        // Spend zMNT
        ui->pushButtonSpendzMNT->setEnabled(false);
        ui->pushButtonSpendzMNT->setToolTip(tr("zMNT is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zMNT
        ui->pushButtonMintzMNT->setEnabled(true);
        ui->pushButtonMintzMNT->setToolTip(tr("PrivacyDialog", "Enter an amount of MNT to convert to zMNT", 0));

        // Spend zMNT
        ui->pushButtonSpendzMNT->setEnabled(true);
        ui->pushButtonSpendzMNT->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
