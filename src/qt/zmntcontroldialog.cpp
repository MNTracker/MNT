...
#include "ui_zmntcontroldialog.h"
...
std::set<std::string> ZMntControlDialog::setSelectedMints;
std::set<CMintMeta> ZMntControlDialog::setMints;

ZMntControlDialog::ZMntControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZMntControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZMntControlDialog::~ZMntControlDialog()
{
    delete ui;
}

void ZMntControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMntControlDialog::updateList()
{
...
void ZMntControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMntControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZMnt_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMntControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZMntControlDialog::GetSelectedMints()
{
...
void ZMntControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
