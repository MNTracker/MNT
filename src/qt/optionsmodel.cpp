...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MNT);
    ...
    if (!settings.contains("nAnonymizeMntAmount"))
        settings.setValue("nAnonymizeMntAmount", 1000);

    nAnonymizeMntAmount = settings.value("nAnonymizeMntAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMntAmount"))
        SoftSetArg("-anonymizemntamount", settings.value("nAnonymizeMntAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mnt.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMntAmount:
            return QVariant(nAnonymizeMntAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMntAmount:
            nAnonymizeMntAmount = value.toInt();
            settings.setValue("nAnonymizeMntAmount", nAnonymizeMntAmount);
            emit anonymizeMntAmountChanged(nAnonymizeMntAmount);
            break;
        case CoinControlFeatures:
