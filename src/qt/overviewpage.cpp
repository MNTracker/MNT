...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::MNT)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sMNTPercentage, QString& szMNTPercentage)
{
    int nPrecision = 2;
    ...
    szMNTPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sMNTPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& earnings, const CAmount& masternodeEarnings, const CAmount& stakeEarnings)
{
    ...
    CAmount mntAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;
    // MNT Watch-Only Balance
    ...
    // Combined balances
    CAmount availableTotalBalance = mntAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // MNT labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, mntAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    QString automintHelp = tr("Current percentage of zMNT.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", Params().ZeroCoinEnabled());
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in mnt.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in mnt.conf");
    }
    ...
    // MNT Available
    bool showMNTAvailable = settingShowAllBalances || mntAvailableBalance != nTotalBalance;
    bool showWatchOnlyMNTAvailable = showMNTAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showMNTAvailable || showWatchOnlyMNTAvailable);
    ui->labelBalance->setVisible(showMNTAvailable || showWatchOnlyMNTAvailable);
    ui->labelWatchAvailable->setVisible(showWatchOnlyMNTAvailable && showWatchOnly);

    // MNT Pending
    bool showMNTPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyMNTPending = showMNTPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showMNTPending || showWatchOnlyMNTPending);
    ui->labelUnconfirmed->setVisible(showMNTPending || showWatchOnlyMNTPending);
    ui->labelWatchPending->setVisible(showWatchOnlyMNTPending && showWatchOnly);

    // MNT Immature
    bool showMNTImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = showMNTImmature || watchImmatureBalance != 0;
    ui->labelImmatureText->setVisible(showMNTImmature || showWatchOnlyImmature);
    ui->labelImmature->setVisible(showMNTImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelWatchImmature->setVisible(showWatchOnlyImmature && showWatchOnly); // show watch-only immature balance

    // MNT Locked
    bool showMNTLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyMNTLocked = showMNTLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showMNTLocked || showWatchOnlyMNTLocked);
    ui->labelLockedBalance->setVisible(showMNTLocked || showWatchOnlyMNTLocked);
    ui->labelWatchLocked->setVisible(showWatchOnlyMNTLocked && showWatchOnly);
    ...
    ui->labelMNTPercent->setVisible(false);
    ui->labelzMNTPercent->setVisible(false);
