...
#if defined(NDEBUG)
#error "MNT cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in umnt) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zMNT is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zMNT spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zMNT without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zMNT spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zMNT spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zMNT spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zmnt");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zMNT mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 0.25 * COIN;
    } else if (nHeight <= 6000 && nHeight > 3000) {
        nSubsidy = 0.5 * COIN;
    } else if (nHeight <= 15000 && nHeight > 6000) {
        nSubsidy = 1 * COIN;
        ...
        nSubsidy = 12 * COIN;
    } else if (nHeight <= 95000 && nHeight > 85000) {
        nSubsidy = 16 * COIN;
    } else if (nHeight <= 105000 && nHeight > 95000) {
        nSubsidy = 20.5 * COIN;
    } else if (nHeight <= 115000 && nHeight > 105000) {
        nSubsidy = 26.65 * COIN;
    } else if (nHeight <= 125000 && nHeight > 115000) {
        nSubsidy = 34 * COIN;
    } else if (nHeight <= 135000 && nHeight > 125000) {
        nSubsidy = 44 * COIN;
    } else if (nHeight <= 145000 && nHeight > 135000) {
        nSubsidy = 58 * COIN;
    } else if (nHeight <= 155000 && nHeight > 145000) {
        nSubsidy = 75 * COIN;
    } else if (nHeight <= 165000 && nHeight > 155000) {
        nSubsidy = 95 * COIN;
    } else if (nHeight <= 175000 && nHeight > 165000) {
        nSubsidy = 125 * COIN;
    } else if (nHeight <= 185000 && nHeight > 175000) {
        nSubsidy = 160 * COIN;
    } else if (nHeight <= 205000 && nHeight > 185000) {
        nSubsidy = 168 * COIN;
    } else if (nHeight <= 225000 && nHeight > 205000) {
        nSubsidy = 175 * COIN;
    } else if (nHeight <= 245000 && nHeight > 225000) {
        nSubsidy = 185 * COIN;
    } else if (nHeight <= 265000 && nHeight > 245000) {
        nSubsidy = 195 * COIN;
    } else if (nHeight <= 285000 && nHeight > 265000) {
        nSubsidy = 205 * COIN;
    } else if (nHeight <= 305000 && nHeight > 285000) {
        nSubsidy = 215 * COIN;
    } else if (nHeight <= 325000 && nHeight > 305000) {
        nSubsidy = 225 * COIN;
    } else if (nHeight <= 345000 && nHeight > 325000) {
        nSubsidy = 235 * COIN;
    } else if (nHeight <= 365000 && nHeight > 345000) {
        nSubsidy = 245 * COIN;
    } else if (nHeight <= 385000 && nHeight > 365000) {
        nSubsidy = 260 * COIN;
    } else if (nHeight <= 405000 && nHeight > 385000) {
        nSubsidy = 275 * COIN;
    } else if (nHeight <= 425000 && nHeight > 405000) {
        nSubsidy = 290 * COIN;
    } else if (nHeight <= 445000 && nHeight > 425000) {
        nSubsidy = 305 * COIN;
    } else if (nHeight <= 465000 && nHeight > 445000) {
        nSubsidy = 320 * COIN;
    } else if (nHeight <= 485000 && nHeight > 465000) {
        nSubsidy = 335 * COIN;
    } else if (nHeight <= 505000 && nHeight > 485000) {
        nSubsidy = 350 * COIN;
    } else if (nHeight <= 525000 && nHeight > 505000) {
        nSubsidy = 365 * COIN;
    } else if (nHeight <= 545000 && nHeight > 525000) {
        nSubsidy = 380 * COIN;
    } else if (nHeight <= 565000 && nHeight > 545000) {
        nSubsidy = 400 * COIN;
    } else if (nHeight <= 585000 && nHeight > 565000) {
        nSubsidy = 	425 * COIN;
    } else if (nHeight <= 768198 && nHeight > 585000) {
        nSubsidy = 500 * COIN;
...
CAmount GetSeeSaw(const CAmount& blockValue, int nHeight)
{
    ...
    return blockValue * ret / 100 ;
}

int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZMNTStake)
{
   return GetSeeSaw(blockValue, nHeight);
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from MNT
...
{
    RenameThread("mnt-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZMNTMinted()
{
...
void RecalculateZMNTSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMNT supply
...
bool RecalculateMNTSupply(int nHeightStart)
{
...
{
    // MNT: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZMNTSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZMNTMinted();
        RecalculateZMNTSpent();
        RecalculateMNTSupply(Params().Zerocoin_StartHeight());
    }

    //Track zMNT money supply in the block index
    if (!UpdateZMNTSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zMNT supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMntSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MNT to zMNT
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zMNT spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zMNT serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zMNT stake block is less than allowed start height", __func__);

    if (CZMntStake* zMNT = dynamic_cast<CZMntStake*>(stake)) {
        CBlockIndex* pindexFrom = zMNT->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zMNT stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zMNT stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zMNT->GetValue()));
        if (nChecksum200 != zMNT->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zMNT->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZMNT() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zMNT fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zMNT mints and %d zMNT spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // MNT: We use certain sporks during IBD, so check to see if they are
