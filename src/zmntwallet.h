...
#ifndef MNT_ZMNTWALLET_H
#define MNT_ZMNTWALLET_H
...
public:
    CzMNTWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMNT(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMNT(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //MNT_ZMNTWALLET_H
