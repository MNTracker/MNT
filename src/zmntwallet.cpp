...
CzMNTWallet::CzMNTWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmnt seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMNTSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMNTSeed_deprecated()) {
                    LogPrintf("%s: Updated zMNT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmnt seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmnt wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMNTWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMNTCount(nCountLastUsed);
    else if (!walletdb.ReadZMNTCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMNTWallet::Lock()
{
    seedMaster = 0;
}

void CzMNTWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMNTWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMNT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMNTWallet::LoadMintPoolFromDB()
{
...
void CzMNTWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMNTWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMNTWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmntTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMNTWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMNT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmntTracker which also adds to database
    pwalletMain->zmntTracker->Add(dMint, true);
...
void CzMNTWallet::SeedToZMNT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMNTWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMNTWallet::GenerateDeterministicZMNT(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMNTWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMNTWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
